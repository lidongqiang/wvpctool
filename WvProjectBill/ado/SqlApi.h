...
typedef struct  
{
    wstring m_strItem[FLAG_WVCNT];
} RECORD;
typedef struct  
{
    string m_strItem[FLAG_WVCNT];
} RECORDA;
class CWvToolDB 
{
public:
...
public:
    ...
    wstring GenerateCodition(const wstring strItem,const wstring strValue)
    {
    ...
    BOOL GetWVkeyAndUpdateFlag(wstring &wvkey);
    BOOL GetWVkey(const TCHAR *strItem,DWORD type,DWORD dwflags,wstring &wvkey);
    BOOL CreateTable(const TCHAR *strCmd);
    BOOL OpenTableForAddNew(std::wstring strTableName);
    BOOL AddSingleItem2Database(RECORDA&vsRecord);
